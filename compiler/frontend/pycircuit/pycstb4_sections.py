...
def _decode_events(blob: bytes, count: int, strings: list[str], max_words: int) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for idx in range(count):
        ...
        pos += max_words * 8
        masks = [int.from_bytes(blob[pos + i * 8 : pos + i * 8 + 8], "little", signed=False) for i in range(max_words)]
        out.append(
            {
                "cycle": int(cycle),
...
def _decode_frames(blob: bytes, count: int, strings: list[str], max_words: int) -> tuple[list[dict[str, Any]], list[str]]:
    errors: list[str] = []
    pos = 0
    out: list[dict[str, Any]] = []
...
def _decode_patterns(blob: bytes, count: int, max_words: int) -> tuple[list[dict[str, Any]], list[str]]:
    errors: list[str] = []
    pos = 0
    out: list[dict[str, Any]] = []
    ...
    for idx in range(count):
        ...
        pos += max_words * 8
        default_words = [int.from_bytes(blob[pos + i * 8 : pos + i * 8 + 8], "little", signed=False) for i in range(max_words)]
        pos += max_words * 8
        out.append(
            {
                ...
                "default_value": _value_from_words(default_words[: int(default_nwords)]),
            }
        )
    return out, errors
...
def inspect_pycstb4_file(path: str | Path, *, registry: SectionRegistry | None = None) -> dict[str, Any]:
    ...
    if directory_end > len(data):
        ...
        for required_kind in sorted(registry.required_kinds()):
            if required_kind not in present_kinds:
                ...
                errors.append(f"missing required section: {required_kind} ({descriptor.name if descriptor else required_kind})")
        for section in sections:
            ...
            for dep in descriptor.dependencies:
                if int(dep) not in present_kinds:
                    ...
                    errors.append(f"section {section.name} requires missing dependency {int(dep)} ({dep_desc.name if dep_desc else dep})")
    ...
    if not errors:
        ...
        if event_section is not None:
            events, decode_errors = _decode_events(_slice_section(data, event_section), event_section.count, strings, header.max_words)
            errors.extend(decode_errors)
            decoded["events"] = events
        frame_section = section_by_kind.get(int(SectionKind.FRAME_TABLE))
        if frame_section is not None:
            frames, decode_errors = _decode_frames(_slice_section(data, frame_section), frame_section.count, strings, header.max_words)
            errors.extend(decode_errors)
            decoded["frames"] = frames
        pattern_section = section_by_kind.get(int(SectionKind.PATTERN_TABLE))
        if pattern_section is not None:
            patterns, decode_errors = _decode_patterns(_slice_section(data, pattern_section), pattern_section.count, header.max_words)
            errors.extend(decode_errors)
