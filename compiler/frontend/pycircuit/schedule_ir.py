    ...
    *,
    ...
    for pid, samples in sorted(samples_by_port.items()):
        ...
        for drop_prefix in range(0, min(4, len(samples) - 16) + 1):
            ...
            if all(value == (0 if ((cyc - phase_cycle) % period) < active_cycles else 1) for cyc, value in candidate):
...
def _pycstb4_frame_count_after_pattern_compaction(schedule_ir: Mapping[str, Any]) -> int:
    ...
    for frame in schedule_ir.get("frames", []):
        ...
        items = [item for item in frame.get("items", []) if isinstance(item, Mapping) and not (kind == "drive_frame" and int(item["port"]) in covered)]
        if items:
            frame_count += 1
    return frame_count
...
def _packed_section(registry: SectionRegistry, kind: SectionKind | int, data: bytes, count: int, *, flags: int = 1) -> PackedSection:
