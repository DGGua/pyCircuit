...
struct CheckCombPartitionsPass
  ...
  void runOnOperation() override {
    ...
    bool hasStampedComb = false;
    function.walk([&](pyc::CombOp comb) {
      hasStampedComb |= hasAnyPartitionAttribute(comb);
    });
    // This pass verifies partition plans, not legacy unpartitioned probe
    // namespaces. Current main permits repeated internal pyc.name values, so
    // only apply the hardened namespace and boundary contract when a plan is
    // actually present (including torn comb-only metadata).
    if (!hasFunctionMetadata && !hasStampedComb)
      return;
