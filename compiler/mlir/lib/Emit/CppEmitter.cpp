...
static LogicalResult emitCombAssign(Operation &op, llvm::raw_ostream &os, NameTable &nt,
                                    CppEmitterPlacementState *ps, ArrayReadEmitState *ars = nullptr) {
  if (ps) {
  ...
  if (auto ar = dyn_cast<pyc::ArrayReadOp>(op)) {
    ...
    // The result may be placement-Local (used only by the comb region's
    // yield): declare it before the assignment, matching dev's local-value
    // emission for ordinary comb ops.
    if (ps && getValueCppStorage(ar.getResult()) == CppStorageKind::Local)
      (void)ps->emitLocalDeclIfNeeded(ar.getResult(), ar.getResult().getType(),
                                      nt.get(ar.getResult()), os);
    std::string key;
  ...
  if (combOps.size() > combChunkNodes) {
    ...
    for (auto &part : parts) {
      ...
      for (Operation *op : part.second) {
        if (failed(emitCombAssign(*op, os, nt, &placement, ars)))
...
static LogicalResult emitFunc(func::FuncOp f, llvm::raw_ostream &os, const CppEmitterOptions &opts) {
  ...
  for (Operation *op : ordered) {
    ...
    if (isa<pyc::ConstantOp,
            ...
            pyc::VAddReduceOp,
            arith::SelectOp>(*op)) {
      if (failed(emitCombAssign(*op, os, nt, nullptr, &ars)))
        return failure();
      continue;
    }
    if (isa<pyc::FifoOp,
