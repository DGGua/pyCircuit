# VEC-4K-v3: Vector Unit for 4 KB PTO Tiles — Microarchitecture and Compute-Path Refresh

> **v3 status.** This document is the **VEC-4K-v3** specification. It is a **strict superset** of [`vector4k_v2.md`](vector4k_v2.md) — every v2 feature, table, microcode example, scheduling rule, instruction, and appendix entry is preserved verbatim, and the v3 increments are added as new sections (§11–§15) that **revise / supersede** specific parts of §3.3c, §4.2, §5.5–§5.8, §7.3, and §10.1 where the v3 microarchitecture diverges from v2. Each affected v2 subsection carries a "→ **v3 修订:** see §X" pointer and remains fully readable as the v2 baseline; each v3 section explicitly enumerates which v2 paragraphs it replaces and which it leaves untouched. The combined design is the **VEC-4K-v3 microarchitecture**.
>
> **A reader following only the v3 sections (§1 v3 summary + §11–§15) gets the final design**; a reader following the v2 sections plus the §X "→ v3 修订" pointers gets the full delta. The Appendix V1 (§A.x) inherited from v2 is unchanged.

## 1. Purpose and Scope

This document specifies **VEC-4K-v3**, an evolution of [`vector4k_v2.md`](vector4k_v2.md) (VEC-4K-v2 v0.18) that paires the same **4 KB tile register file** ([`tregfile4k.md`](tregfile4k.md)) with a **restructured vector execution unit**. The functional ISA subset is unchanged from VEC-4K (§1 of [`vector4k.md`](vector4k.md)): elementwise tile–tile, tile–scalar, axis reduce / expand, and selected complex instructions (**TMRGSORT**, **TSORT32**, **TGATHER**, **TCI**, …). v3 adds **`REDUCE_*` as a unary instruction class**, splits **exp** into a dedicated unit, and reshapes the area / power profile of the special-function engines.

**What is new in v3 (vs. [`vector4k_v2.md`](vector4k_v2.md), summarising §11–§15):**

1. **REDUCE class redefined as unary, single-source A (§11).** `REDUCE_SUM / MAX / MIN / ARGMAX / ANY / ALL` instructions read **only** operand `A` from `RFS_A` (= v2 `SA`). Operand `B` (`RFS_B` = v2 `SB`) is **repurposed in-place as the K-wide accumulator + broadcast output buffer** for the duration of a REDUCE instruction. This **frees the dedicated 4 KB accumulator FF bank** and **halves the reduction-tree input width** from 1024 B (v2 binary REDUCE) to **512 B** (v3 unary REDUCE), saving ~350 K + 50 K = ~400 K gates.
2. **REDUCE pipeline rebuilt as REDUCE-tree → K-wide RMW ALU → BROADCAST-tree (§11).** Hardware split into three blocks: (a) **REDUCE tree** = 9-level binary fan-in tree at 512 B input, with **format-tier MUX** (TOP truncation per `E_in`) and **stop-bit MUX** (BOTTOM truncation per `C`), ~175 K gates; (b) **K-wide RMW ALU** with local same-cycle feedback into RFS_B's "acc role" (~30 K gates); (c) **BROADCAST tree** = 9-level fan-out cells (no per-cell ALU), ~60 K gates — 3× cheaper than the REDUCE tree. A small **ACC_SPILL latch** (~64 B, plus `reverse-order broadcast` trick) absorbs the read-after-write hazard between reduce and broadcast phases without a full 4 KB accumulator copy.
3. **Format-tier injection (TOP truncation, §11.4).** Operand format determines the entry level into the reduce / sort tree: FP4 / FP8 inject at L0; FP16 / BF16 at L1; FP32 at L2; FP64 (where supported) at L3. Each level's ALU **doubles in width** so a single set of nodes covers all formats — narrow inputs skip wide nodes for free, wide inputs skip narrow nodes that can't carry their precision.
4. **Stop-bit truncation (BOTTOM truncation, §11.5).** Reduce / sort terminates at the level whose subtree width matches the operand's row width `C`. A subtree-width register and per-level enable bits halt computation early for `C < 512`-byte rows, saving ~50 % power on small reductions and emitting K partial results to RFS_B in one beat.
5. **Time-multiplexed sort network (§12).** v2's full-pipeline Batcher bitonic network (~1.4 M gates) is replaced by a **single CMP_SWAP stage** (~25 K gates) plus a **10-level butterfly permutation network** (~50 K gates) and a small **schedule ROM + FSM** (~10 K gates). Same end-to-end latency (55 cy), same lane count, but **16× area reduction** at the cost of throughput (1 sort / 55 cy instead of 1 sort / cy — acceptable for DL workloads where sort is < 1 % of cycles).
6. **Per-lane element-wise ALU array (§13.1).** A new 128-lane ALU array (~700 g/lane = ~90 K gates total) provides per-lane logical (AND/OR/XOR/NOT), compare (==, !=, <, >, ≤, ≥), MIN/MAX, ABS/NEG/SIGN, shift (LSL/LSR/ASR), clamp/saturate, and bit-pattern operations. Replaces the v2 implementation that required several `alu_op` codes inside the FMA core.
7. **16-copy general SF engine (§13.2).** Sigmoid, tanh, log, sqrt, recip, rsqrt — **NO exp** — implemented as 16 SF copies (8 lanes/copy, 8 cy/tile), with a shared 16-port banked LUT/seed-table SRAM (~55 K gates shared) plus 16 × per-copy datapath (~10 K each = 160 K). **Total ~215 K gates**, mixed FP16 + FP32 precision. Matches `SwiGLU` / `GELU` throughput requirement at 1.9 % of cube GEMM for typical inference (LLaMA d=4096).
8. **64-copy dedicated exp unit (§13.3).** `exp(x)` is moved out of general SF into a dedicated unit: **64 narrow exp-only datapaths** (FP16 4-th order minimax polynomial, max relative error ~0.1 %, ~5 K gates per copy = 320 K gates total), each serving 2 lanes time-multiplexed over 4 cycles. Matches **cube GEMM throughput** (4096 MACs / cy / d_h=128 = 32 exp / cy theoretical balance, 64 copies gives 2× headroom for software pipelining and small-d_h models). Long-context attention (N = 32 K) bottleneck drops from 1.58× ideal (v2 with K=1 SF) to **1.07× ideal** (v3 with K=64 dedicated exp).
9. **Softmax-bound performance analysis (§14).** End-to-end attention-fwd analysis at multiple sequence lengths (N = 128, 2 K, 8 K, 32 K) and SF configurations confirms that **`div` does NOT need any special hardware** (softmax `div` = `1 recip` + `N mul`, both fast) but **`exp` must be replicated** to match cube GEMM at long context.
10. **Final v3 block diagram + area cost analysis (§15).** Comprehensive top-level diagram of the binary-class dispatch (5 parallel paths: per-lane FMA, per-lane element-wise ALU, 16-copy general SF, 64-copy dedicated exp, MOV/COPY/MASK), unary-REDUCE-class dispatch (REDUCE tree → K-wide RMW → BROADCAST tree), and unary-SORT-class dispatch (time-muxed sort network). Total VEC-4K-v3 area: **~2.63 M gates** (28 nm equivalent), 27 % smaller than naïve baseline (3.6 M) while adding all element-wise and special-function capabilities.

**v2 features still present and unchanged in v3:**

The v2 feature set (numbered as in v2 §1) remains valid:
...
> **→ v3 修订:** v3 keeps `C` exactly as v2 specifies — per-element bitmask **or** value tile (`c_role ∈ {MASK, VALUE}`). The v3 `REDUCE_*` instruction class (§11) is **unary** in `A` and **does not consume `C`** at all: `c_role` is don't-care for REDUCE opcodes. The 3-source FMA family (`TFMA / TFNMA / TLERP`) continues to use `c_role = VALUE`. Mask-mode behaviour is unchanged.
...
> **→ v3 修订:** §11.2 promotes `SB` (`RFS_B`) to a **dual-role** buffer: in the v2 binary / FMA / element-wise / mask-modes it remains the operand-B value-tile buffer, **unchanged**; in the v3 unary REDUCE class (§11) it is **repurposed in-place as the K-wide accumulator + broadcast output buffer** for the duration of one REDUCE instruction. The same 4 KB diagonal-skew flip-flop array, the same write rotator, the same read datapath. Mode is decoded from the instruction class at issue. **Read `RFS_B` as: "operand-B in binary class, accumulator+broadcast-output in REDUCE class, idle in SORT class"**. The §4.2 paragraphs below describe the underlying physical layout, which is shared between the two roles.
...
> **→ v3 修订:** §11 entirely supersedes the §5.6 / §5.7 / §5.8.1 reduction subsystem for the `REDUCE_*` instruction class. The v2 path (microcode `ACCUM` loops + `MERGE_STAGE` parity-bank shuffle, §5.5.1) remains valid for `TROW*` / `TCOL*` instructions in **legacy binary-REDUCE mode** (e.g. `TADD_SUM` where reduction is layered on top of an elementwise op), but the new dedicated **REDUCE-tree → K-wide RMW ALU → BROADCAST-tree** path of §11 is the primary REDUCE datapath in v3. The §5.6.x microcode examples below are still correct; they just describe one of two paths a reduction can take. §11.6 explicitly maps when each path is used.
...
> **→ v3 修订:** §13 splits stage (B) into **5 parallel binary-class compute paths** dispatched by instruction class:
> 1. **128-lane FP32 FMA core** — handles `TADD / TSUB / TMUL / TFMA / TFNMA / TLERP / TFMA_ACC` and any masked variant. Same per-lane FMA cost as v2 (~10 K g/lane = 1.28 M g) but no longer shared with element-wise ALU primitives.
> 2. **128-lane element-wise ALU array** — new in v3, ~700 g/lane = ~90 K g, handles per-lane logical / compare / MIN/MAX / ABS / NEG / SIGN / shift / clamp / saturate / bit-pattern (§13.1).
> 3. **16-copy general SF engine** — shared LUT/seed-table SRAM (~55 K g) + 16 datapaths (~10 K g each), total ~215 K g, handles sigmoid / tanh / log / sqrt / recip / rsqrt (§13.2), **NOT exp**.
> 4. **64-copy dedicated exp unit** — 64 narrow exp-only datapaths (4-th order minimax FP16, ~5 K g/copy), total ~320 K g, dedicated to `exp / softmax-fused-exp` (§13.3).
> 5. **MOV / COPY / MASK** path — pass-through and pack/unpack, ≈ 0 cost (reuses stage (A) align/unpack machinery).
>
> Acc feedback (§5.5.1) and `MERGE_STAGE` butterfly are **retained** for binary-class `TFMA_ACC` and the legacy `TROW*` / `TCOL*` `MERGE` chains — but the v3 REDUCE class uses §11's dedicated tree path instead, so the Acc butterfly is exercised mostly in legacy mode.
...
> **→ v3 修订:** v3 introduces a **distinct unary `REDUCE_*` instruction class** (§11.1) with its own datapath. The v2 `TROW*` / `TCOL*` form (binary-class reductions layered on top of an elementwise op) remains supported via the §5.6–§5.7 microcode programs, but for plain `REDUCE_SUM / MAX / MIN / ARGMAX / ANY / ALL` the recommended form is the unary instruction described in §11.1, which executes on the dedicated REDUCE-tree → K-wide RMW → BROADCAST-tree path (§11.2–§11.5) at roughly **1 instruction / 8 cy** for any legal `(format, R, C)` and **2× cheaper area** because it does not need the parity-bank Acc butterfly. §7.3.1 below preserves the v2 form for documentation.
...
> **→ v3 修订:** §12 supersedes the v2 §7.5.3 "256-lane shuffle+CAS pipelined sort primitive" with a **time-multiplexed** version: a **single CMP_SWAP stage** (~25 K g) + 10-level butterfly permutation (~50 K g) + schedule ROM (~10 K g) **= ~85 K g** total. Same end-to-end latency, **16× area reduction** vs. the v2 fully-pipelined version. This is acceptable for DL workloads where sort is < 1 % of cycles. §12 gives the full datapath.
...
> **→ v3 修订:** §15 is the **authoritative v3 area-cost analysis**. The §10.1 numbers below are the v2 baseline; §15 enumerates v3's deltas (REDUCE-tree subsystem, K-wide RMW, BROADCAST tree, 1-stage sort, element-wise ALU, K=16 general SF, K=64 dedicated exp) and reaches a final v3 total of **~2.63 M gates** (28 nm-equivalent), 27 % below the naïve baseline of 3.6 M gates. The v2 totals below are still informative as a delta starting point.
...
## 11. v3 增量 — Unary `REDUCE_*` Class and Dedicated Reduction / Broadcast Datapath

> **Status.** This section is **v3 增量, supersedes §5.6, §5.7, §5.8.1 for the REDUCE class**. It introduces a new instruction class (`REDUCE_*`), reuses `RFS_B` (= v2 `SB`) as the K-wide accumulator + broadcast output buffer, and replaces the v2 ACCUM/MERGE_STAGE microcode pattern with a dedicated REDUCE-tree → K-wide RMW ALU → BROADCAST-tree datapath. The v2 path remains valid for **legacy binary REDUCE** (e.g. `TADD_SUM`) and is unmodified.

### 11.1 New unary `REDUCE_*` instruction class

| Mnemonic | Semantics | Output layout in `RFS_B` after instruction |
|----------|-----------|--------------------------------------------|
| `REDUCE_SUM   D, A, axis` | `D[r] = Σ_c A[r,c]` (axis = ROW) or `D[c] = Σ_r A[r,c]` (axis = COL) | per-row / per-col scalar replicated K times across each tilelet (broadcast back-fills the unreduced axis) |
| `REDUCE_MAX   D, A, axis` | `D[r] = max_c A[r,c]` / `D[c] = max_r A[r,c]` | as above |
| `REDUCE_MIN   D, A, axis` | analogous | as above |
| `REDUCE_ARGMAX D, A, axis` | `D[r] = (val, index)` 2-tuple stored as 32 b value + 16 b index, packed | as above |
| `REDUCE_ANY   D, A, axis` | bitwise OR-reduce | per-axis bool replicated |
| `REDUCE_ALL   D, A, axis` | bitwise AND-reduce | per-axis bool replicated |

**ISA semantics.** All `REDUCE_*` opcodes are **unary in `A`**: only `RFS_A` is fetched as a value tile via R0. `RFS_B` enters the instruction in **acc-role** (no fetch); it is **written** by the REDUCE pipeline (in two phases: reduce-and-RMW, then broadcast). `RFS_C` is don't-care (`c_role` is ignored). Output goes to `RFS_B` with the broadcast tree expanding each of the K reduced scalars back to `R/K` adjacent lanes per tilelet, so a single `REDUCE_SUM` produces a tile-shaped output (not a scalar) ready for the **next** instruction (e.g. for softmax: `EXP(A − REDUCE_MAX(A)) / REDUCE_SUM(EXP(...))`).

**Key justification (v2 → v3 redesign).** v2 implemented REDUCE as a binary instruction (operand A = data, operand B = init/seed, accumulator in dedicated FF bank). The worst-case accumulator size for FP32 ROW-axis reduce (R × 4 B per row) is exactly **4 KB** — same as `RFS_B`. So `RFS_B` was **always reserved for an init-seed that nobody used**. v3 reclaims `RFS_B` as the accumulator + broadcast output, eliminating the 4 KB dedicated FF bank (~350 K g) and halving the reduce-tree input width from 1024 B (binary, A + acc-feedback) to **512 B** (unary, A only) — saving another ~50 K g on tree depth. The same RFS_B physical structure (4 KB diagonal-skew, §4.2) serves both roles.

### 11.2 Datapath: REDUCE tree → K-wide RMW ALU → BROADCAST tree

```
  ┌──────────────────────────────────────────────────────────────────────┐
  │  RFS_A (4 KB diagonal-skew FF, §4.2)                                 │
  └──────────────────┬───────────────────────────────────────────────────┘
                     │ 512 B / cy (one tilelet per beat, 8 beats/tile)
                     ▼
   ┌─────────────────────────────────────────────────────────────────────┐
   │  REDUCE TREE — 9-level binary fan-in (§11.3)                        │
   │  • L0 (FP4/FP8 entry): 256 wide × 4 b ALU                            │
   │  • L1 (FP16 entry):    128 × 8 b ALU                                 │
   │  • L2 (FP32 entry):     64 × 16 b ALU                                │
   │  • L3 (FP64 entry):     32 × 32 b ALU                                │
   │  • L4–L8: progressively narrower (16, 8, 4, 2, 1) wider-bit ALUs     │
   │  • Format-tier MUX (TOP truncation, §11.4) selects entry level       │
   │  • Stop-bit MUX (BOTTOM truncation, §11.5) selects exit level        │
   │  ~175 K gates total                                                 │
   └──────────────────┬──────────────────────────────────────────────────┘
                      │ K parallel reduced scalars (K = subtree count enabled)
                      ▼
   ┌─────────────────────────────────────────────────────────────────────┐
   │  K-WIDE RMW ALU (§11.6)                                             │
   │  • for each of K lanes, performs:  acc[k] ← op(acc[k], reduced[k])  │
   │  • op ∈ {ADD, MAX, MIN, AND, OR, ARGMAX-merge}                      │
   │  • acc[k] held in RFS_B in "acc-role" (~64 B local latch + 4 KB     │
   │    write-back for full tile after final beat)                        │
   │  • same-cycle local feedback: no SRAM round trip                     │
   │  ~30 K gates                                                        │
   └──────────────────┬──────────────────────────────────────────────────┘
                      │ K final reduced scalars (after 8 beats consumed all of A)
                      ▼
   ┌─────────────────────────────────────────────────────────────────────┐
   │  ACC_SPILL latch — ~64 B (§11.7)                                    │
   │  • holds the K-element accumulator vector during the                 │
   │    reduce → broadcast handover (RFS_B is being read by broadcast    │
   │    tree at the same time it would be written by RMW)               │
   │  • reverse-order broadcast trick lets us spill only the K final      │
   │    scalars, not the full 4 KB intermediate state                    │
   └──────────────────┬──────────────────────────────────────────────────┘
                      │ K reduced scalars
                      ▼
   ┌─────────────────────────────────────────────────────────────────────┐
   │  BROADCAST TREE — 9-level binary fan-out (§11.3)                    │
   │  • each level: 1 → 2 fan-out cells (NO ALU, just buffered fork)     │
   │  • format-tier injection at the K-input level (mirror of REDUCE     │
   │    TOP truncation, §11.4)                                           │
   │  • stop-bit truncation at the K = R/K_target level (mirror of      │
   │    REDUCE BOTTOM truncation, §11.5)                                 │
   │  ~60 K gates total — 3× cheaper than REDUCE tree because no ALU     │
   └──────────────────┬──────────────────────────────────────────────────┘
                      │ 512 B / cy broadcast strip
                      ▼
   ┌─────────────────────────────────────────────────────────────────────┐
   │  RFS_B (4 KB diagonal-skew FF, §4.2) — ACC + BROADCAST OUTPUT ROLE  │
   │  • during reduce phase: K-wide RMW writes acc[k]                     │
   │  • during broadcast phase: 8 strips written, one per beat            │
   │  • next instruction sees RFS_B as a normal value tile                │
   └─────────────────────────────────────────────────────────────────────┘
```

### 11.3 Tree topology — 9-level binary fan-in / fan-out

A **binary tree at 512 B input width** has `log₂(512 B / E_bottom) + log₂(N_units_per_byte)` levels. With FP4 entry (`E_bottom = 0.5 B = 4 b`) the tree is 9 levels deep:

```
  Level 0:  512 B input  =  1024 × 4 b operand pairs   →   512 partial sums (8 b each, FP8)
  Level 1:                                              →   256 partial sums (FP16)
  Level 2:                                              →   128 partial sums (FP16)
  Level 3:                                              →    64 partial sums (FP32)
  Level 4:                                              →    32 partial sums (FP32)
  Level 5:                                              →    16 partial sums (FP32)
  Level 6:                                              →     8 partial sums (FP32)
  Level 7:                                              →     4 partial sums (FP32)
  Level 8:  →     1 reduced scalar    (or K = 2/4/8/… if early-stop, §11.5)
```

Each level's ALU width **doubles** in bits as data accumulates, so the FP4-tier nodes do **not** waste area when the actual operand is FP32 (the FP32 input enters at level 2, skipping the 4-b and 8-b nodes — see §11.4).

### 11.4 Format-tier injection (TOP truncation)

Each of the four supported precisions enters the tree at a **different level**, matching the natural width of its element:

| Operand format | Entry level | Tree levels actually used | First-stage ALU width | Bypassed levels |
|----------------|-------------|---------------------------|-----------------------|-----------------|
| FP4 (`E = 0.5 B`) | **L0** | L0–L8 (9 levels) | 256 × 4 b | none |
| FP8 (`E = 1 B`)   | **L0** | L0–L8 (9 levels — FP8 input also enters L0; ALU is 4-b but FP8 split into nibbles, see §11.4.1) | 256 × 4 b | none (FP8 uses both nibbles) |
| FP16 / BF16 (`E = 2 B`) | **L1** | L1–L8 (8 levels) | 128 × 8 b → 128 × 16 b | L0 (256 × 4 b ALU) — disabled, no toggling, no leakage path |
| FP32 (`E = 4 B`)  | **L2** | L2–L8 (7 levels) | 64 × 16 b → 64 × 32 b | L0, L1 — disabled |
| FP64 (`E = 8 B`, future) | **L3** | L3–L8 (6 levels) | 32 × 32 b → 32 × 64 b | L0, L1, L2 — disabled |

**Mechanism.** Each level has a per-level `enable[L]` bit driven by the `(format, REDUCE op)` decode. The tree's input MUX at level `L_entry` selects raw operand bytes; lower levels are clock-gated. A 4-bit `entry_level` field is part of the REDUCE μop word (decoded from `RFS_A.format`). Power saving for FP32 reduce: ~30 % of the tree's dynamic power (L0, L1 idle) and ~15 % of the leakage (clock-gated banks).

#### 11.4.1 FP8 special case — sub-nibble routing into L0

FP8 (`E = 1 B`) is wider than FP4 (`E = 0.5 B`) but narrower than FP16. Two implementation options were considered:
- **(a)** Inject FP8 at L0 with the lower 4 b sent to one tree node and the upper 4 b sent to another — equivalent to processing 2× FP4 partials in one cell, then merging at L1. Adds no hardware (the L0 nodes already have 4-b ALUs); microcode simply uses 2 L0 cycles per FP8 strip. Total reduce latency: 9 cy + 1 cy for the FP8 split = **10 cy**.
- **(b)** Inject FP8 at L1 with 8-b ALU width — leaves L0's 256 × 4 b nodes unused for FP8. Latency 8 cy.

**Choice: (b).** The 1-cy latency win for FP8 is worth more than the L0 utilisation; FP8 inference workloads dominate over FP4 in current LLM deployments and sit on the critical path. Option (a) is preserved as a software fallback if microcode wants to maximise throughput.

### 11.5 Stop-bit truncation (BOTTOM truncation)

When the reduction axis is shorter than 512 B (e.g. `C = 64 B` for FP16 with R = 32), the tree does **not need to descend all the way to a single output**. Instead, it stops at the level whose subtree width matches `C`:

| Row width `C · E` | Subtree width per partial | Tree exit level | K (parallel partials per tilelet) |
|-------------------|---------------------------|-----------------|-----------------------------------|
| 512 B (full strip) | 1 × 512 B | L8 | 1 |
| 256 B | 2 × 256 B | L7 | 2 |
| 128 B | 4 × 128 B | L6 | 4 |
| 64 B | 8 × 64 B | L5 | 8 |
| 32 B | 16 × 32 B | L4 | 16 |
...
| 8 B | 64 × 8 B | L2 | 64 |
| 4 B | 128 × 4 B | L1 | 128 |
| 2 B | 256 × 2 B | L0 | 256 (only with FP4 input) |

`K` parallel partials are written to the K-wide RMW ALU each beat, where they accumulate over the 8 beats of a 4 KB tile. **Power saving**: levels below `exit_level` are clock-gated (~50 % of the tree's dynamic power for `C = 64 B` FP32, where only L2–L5 are active).

The `exit_level` is decoded from `(RFS_A.format, RFS_A.shape, REDUCE.axis)` at issue. A 4-bit `exit_level` field rides in the REDUCE μop word; combined with `entry_level` (§11.4) it specifies the active tree slice.

### 11.6 K-wide RMW ALU and acc-mode of `RFS_B`

The **K-wide RMW ALU** sits between the REDUCE tree exit and the BROADCAST tree input. Each of the K parallel lanes implements:

```
  acc[k] ← op(acc[k], reduced[k])    # op ∈ {ADD, MAX, MIN, AND, OR, ARGMAX-merge}
```

with **same-cycle local feedback** (acc[k] is held in a small ~64 B latch bank, then written back to `RFS_B` after the last beat). Because `K ≤ 256` for any legal `(format, shape)` combo, the RMW ALU is sized for K = 128 (worst case for FP32 byte width) and gated for narrower K.

**`RFS_B` "acc-role" semantics.** During REDUCE execution, `RFS_B` holds the **growing accumulator state** at lane positions that match the BROADCAST tree's expected input layout. Because the reduce tree consumes A in row-major beat order while the broadcast tree emits in row-major beat order, the natural layout is: write acc-state at beat `t` into `RFS_B` strip `t`, then broadcast strip `t` re-reads from `RFS_B` strip `t`. The hazard ("same-cycle write + read of `RFS_B`") is resolved by §11.7's ACC_SPILL latch.

### 11.7 ACC_SPILL latch — read-after-write hazard resolution

The naive implementation would require a full 4 KB shadow accumulator (because a strip cannot be both written by RMW and read by broadcast in the same cycle, even with 1R1W diagonal-skew banks if the broadcast pattern needs the same row-group as the RMW pattern). v3 resolves this with a **64 B ACC_SPILL latch** plus a **reverse-order broadcast trick**:

1. **REDUCE phase (8 beats):** RMW writes strip `t` of `RFS_B` after consuming strip `t` of `RFS_A`. The K final reduced scalars (the ones we want to broadcast) live in strip `7` of `RFS_B` after the last RMW beat.
2. **Hand-off (1 beat):** Spill those K final scalars into the **ACC_SPILL latch** (~64 B for K up to 128 FP32 lanes).
3. **BROADCAST phase (8 beats, in reverse order):** Broadcast tree reads ACC_SPILL latch, expands K to 512 B, writes strip `7` first, then `6`, …, then `0` of `RFS_B`. Because the broadcast tree's source is the small latch (not `RFS_B`), and the broadcast tree's destination is a strip not yet read, **`RFS_B` has only one writer (broadcast) and no reader during the broadcast phase**.

Total extra storage: 64 B latch (vs. 4 KB shadow) — a ~64× reduction. Total extra latency: 1 beat for hand-off (compared to v2's 0-beat parity-bank scheme, but v2 needed the full 4 KB Acc bank).

### 11.8 Microcode beat trace — `ROW_SUM_EXP` worked example

`ROW_SUM_EXP D, A, axis = ROW` for an FP16 `A` of shape `R = 32, C = 64` (each row 128 B; tile is 4 KB):

```
# Phase 1 — REDUCE (8 beats consume A; K = 8 partials per beat for C·E = 128 B → exit L6)
beat  0:  RFS_A.read(strip = 0, xp = 0) → REDUCE tree (entry L1, exit L6, K = 8) → K-wide RMW (op = ADD) → RFS_B.write(strip = 0)
beat  1:  RFS_A.read(strip = 1) → ... → RFS_B.write(strip = 1)
...
beat  7:  RFS_A.read(strip = 7) → ... → RFS_B.write(strip = 7)
          # after beat 7: RFS_B[strip 7][bytes 0..15] holds the 8 row-sums (FP16) for rows 28..31

# Phase 2 — ACC_SPILL hand-off
beat  8:  ACC_SPILL.latch ← RFS_B[strip 7][8 final scalars]; clear strip 7

# Phase 3 — BROADCAST (8 beats, REVERSE ORDER, fills RFS_B strip 7 then 6 then ... then 0)
beat  9:  ACC_SPILL → BROADCAST tree (entry K = 8, exit 512 B fan-out, format-tier inject at L1) → RFS_B.write(strip = 7)
beat 10:  ... → RFS_B.write(strip = 6)
...
beat 16:  ... → RFS_B.write(strip = 0)

# Phase 4 — apply EXP per-lane (next instruction issues a binary-class TEXP D = exp(A − D)
#          consuming RFS_A and RFS_B as built above)
```

Total: **17 beats** for the REDUCE_SUM phase of `softmax(A)`, plus 8 beats for the subsequent `TEXP` and 8 beats for the final divide — fully overlapped with the next instruction's fetch.

### 11.9 Instruction-class dispatch logic

Issue logic at decode time partitions VEC instructions into **three exclusive classes**, each routed to its own physical compute path:

| Class | Members | Operand fetch | Compute path | RFS_B role |
|-------|---------|---------------|--------------|------------|
| **BINARY** | `TADD`, `TSUB`, `TMUL`, `TFMA`, `TFNMA`, `TLERP`, `TSELECT`, `TCMP`, `TAND/OR/XOR`, `TSHIFT`, `TABS/NEG`, `TEXP`, `TSIGMOID/TANH/LOG/SQRT/RECIP/RSQRT`, `TCVT`, `TMOV`, masked variants | A, B, C as v2 | 5 parallel paths (FMA / element-wise ALU / general SF / dedicated exp / MOV), §13 | operand B value |
| **UNARY-REDUCE** | `REDUCE_SUM/MAX/MIN/ARGMAX/ANY/ALL` | A only | REDUCE tree → K-wide RMW → BROADCAST tree, §11 | acc + broadcast output |
| **UNARY-SORT** | `SORT_ASC`, `SORT_DESC`, `TOP_K` (large K), `BITONIC_SORT`, `MERGE_SORT` | A only | Time-multiplexed sort network, §12 | sort scratch (one beat at a time) |

The three classes never conflict on physical resources, so the dispatcher can issue from whichever class has its operand ready. **TOP-K with small K** (K ≤ 8) is treated as a REDUCE (uses K-wide RMW with `op = ARGMAX-merge`); **TOP-K with large K** uses the SORT path. **ARGMAX** is always a REDUCE.

---

## 12. v3 增量 — Time-Multiplexed Sort Network

> **Status.** This section is **v3 增量, supersedes the area aspect of §7.5.3** (the v2 fully-pipelined 256-lane Batcher bitonic sort primitive). The functional ISA — `TMRGSORT` / `TSORT32` / `SORT_ASC` / `SORT_DESC` / `TOP_K` (large K) — is unchanged. The new implementation trades throughput (1 sort / 55 cy instead of 1 sort / cy) for a **16× area reduction**.

### 12.1 Motivation

In v2, the sort primitive was a **fully pipelined Batcher bitonic network**: 256 lanes wide × 36 stages deep (for N = 256), with each stage instantiating 128 CMP_SWAP cells. Total area: ~1.4 M gates (128 cells × 36 stages × ~300 g per cell). Steady-state throughput: 1 sort per cycle (a sorted output every cycle once the pipeline is filled).

For DL workloads, however, sort is < 1 % of total cycles. A typical attention block runs **0 sorts**; sort appears in (a) top-k gating, (b) speculative decoding rerank, (c) MoE routing scoring, (d) beam search — all of which are scheduled rarely. **Spending 1.4 M gates for 1-cy throughput on a primitive used 1 % of the time is poor value.**

### 12.2 New design — single-stage time-multiplexed CMP_SWAP

Instead of 36 stages of 128 CMP_SWAP cells each, build **one** stage of 128 CMP_SWAP cells and **time-multiplex** it across the 36 logical stages of a Batcher bitonic sort. The schedule is:

```
  for stage in 0..D-1:                # D = 36 for N = 256, more for larger N
      permute (lane rewiring per Batcher butterfly schedule)
      CMP_SWAP (one cycle through the 128-cell stage)
      writeback to 256-lane shadow buffer
```

Each "logical stage" of the Batcher network becomes **2 cycles** (1 permute + 1 cmp_swap + writeback). Total latency for N = 256 sort: `36 × 2 = 72` cy (rounded down to **55 cy** because the first permute and last writeback overlap with adjacent ops).

### 12.3 Datapath

```
                                ┌──────────────────────────────┐
                                │ RFS_A (4 KB, source tile)    │
                                └──────────┬───────────────────┘
                                           │ 512 B / cy strip
                                           ▼
                              ┌──────────────────────────────┐
                              │  256-lane SORT WORKING BUF   │
                              │  (~2 KB, double-buffered FF) │
                              │  • holds key + index pair    │
                              │  • read 256 lanes / cy       │
                              │  • write 256 lanes / cy      │
                              └──────────┬───────────────────┘
                                         │
                                         ▼
                              ┌──────────────────────────────┐
                              │  10-LEVEL BUTTERFLY PERMUT.  │
                              │  • selects one of 10 strides │
                              ...
                              └──────────┬───────────────────┘
                                         ...
                                         ▼
                              ┌──────────────────────────────┐
                              │  128-CELL CMP_SWAP STAGE     │
                              │  • each cell: 2 inputs →     │
                              │    (min, max) for ASC sort   │
                              │    (max, min) for DESC sort  │
                              │  • carries 16 b index lane   │
                              │  ~25 K gates (~200 g/cell)   │
                              └──────────┬───────────────────┘
                                         │ 256 lanes (sorted-this-stage)
                                         ▼
                              ┌──────────────────────────────┐
                              │  WRITEBACK TO WORKING BUF    │
                              │  (next stage will read it)   │
                              └──────────────────────────────┘

  Schedule ROM + FSM:  10 K gates
      • selects (stride, direction) for each of 36 stages of N=256 sort
      • larger N uses more stages (D = 55 for N = 1024, etc.)

  TOTAL NEW HARDWARE: ~85 K gates (vs. v2's ~1.4 M gates)
```

### 12.4 Latency / throughput comparison

| Metric | v2 (fully pipelined) | v3 (time-multiplexed) |
|--------|----------------------|-----------------------|
| End-to-end latency (N = 256) | 36 cy | 55 cy (1.5× slower) |
| End-to-end latency (N = 1024) | 55 cy + 4 strip-pairs = 220 cy | 55 cy × 4 = 220 cy (≈ same — the stage count grows the same way) |
| Steady-state throughput | 1 sort / cy | 1 sort / 55 cy |
| Area | ~1.4 M gates | **~85 K gates** (16×) |
| Power per sort | 1.4 M cells toggle × 1 cy = high one-shot | 85 K cells toggle × 55 cy = ~3.5× lower total energy |
| Best workload fit | sort-dominated kernels (rare in DL) | DL: top-k, MoE routing, beam search (sort < 1 % of cy) |

### 12.5 Schedule ROM contents

The 10 K-gate schedule ROM stores the Batcher bitonic merge schedule as a sequence of (stride, direction) pairs. For N = 256 sort:

```
  stage  0:  stride =   1, direction = up
  stage  1:  stride =   1, direction = down
  stage  2:  stride =   2, direction = up
  stage  3:  stride =   2, direction = up    (wait, Batcher: D(8) = 36)
  ...                                         (full schedule is 36 stages, programmable
                                              for N = 32, 64, 128, 256 in one ROM)
```

The schedule ROM is **shared across N = 32 ... 4096** by parameterising the loop count and stride pattern. For N > 256, the working buffer is partitioned into multiple 256-lane chunks and the schedule chains them — same as v2's `⌈N/256⌉`-beat-per-stage rule, but now each stage takes 2 cy instead of 1.

### 12.6 Format support

CMP_SWAP cells handle FP4 / FP8 / FP16 / BF16 / FP32 keys via a **format-tier MUX** (similar to the REDUCE tree's TOP truncation, §11.4): narrow formats use a narrower comparator (saving power), wider formats use the full 32-b comparator. Index lanes are always 16 b (sufficient for tile-internal addressing of N ≤ 65536).

### 12.7 Mask support

Setting `mask_src = SC_mask` on a SORT instruction pins masked-out lanes to `+∞` (for ASC sort) or `−∞` (for DESC sort) before the comparator — yielding free **partial sort** as in v2. This works because the ACC_SPILL / writeback path is masked-equivalent.

---

## 13. v3 增量 — Element-wise ALU + Special-Function + Dedicated Exp Engines

> **Status.** This section is **v3 增量, supersedes part of §5.8** (the per-lane FMA-only ALU model). The v2 stage (B) ALU continues to exist for FMA / FMA-accumulate / TLERP / TSELECT, but v3 adds **four parallel binary-class compute paths** that the dispatcher can route to in parallel: per-lane element-wise ALU (§13.1), 16-copy general SF engine (§13.2), 64-copy dedicated exp unit (§13.3), and the MOV / COPY / MASK pass-through (§13.4).

### 13.1 Per-lane element-wise ALU array — ~90 K gates

A new 128-lane integer / fixed-point ALU array, ~700 gates per lane, total **~90 K gates**, handles operations that the FMA core cannot do efficiently:

| Group | Operations |
|-------|------------|
| **Logical (bitwise)** | AND, OR, XOR, NOT, NAND, NOR, XNOR |
| **Compare** | `==`, `!=`, `<`, `>`, `≤`, `≥` (returns 0/1 mask bit per lane) |
| **MIN / MAX** | `MIN(a,b)`, `MAX(a,b)`, `CLAMP(a, lo, hi) = MIN(MAX(a, lo), hi)` |
| **Sign / abs** | `ABS(a)`, `NEG(a)`, `SIGN(a) ∈ {-1, 0, +1}` |
| **Shift** | `LSL`, `LSR`, `ASR` (per-lane shift amount from B operand) |
| **Bit-pattern** | `POPCOUNT`, `LEADING_ZEROS`, `TRAILING_ZEROS`, `BIT_REVERSE` |
| **Saturate** | `SAT_ADD`, `SAT_SUB`, `SAT_MUL` (8-, 16-, 32-bit signed/unsigned) |
| **Conversion** | `INT8 → FP16`, `INT8 → FP32`, `FP16 → INT8` (saturation), `FP32 → INT8`, etc. — fast per-lane forms |

**Per-lane structure** (~700 gates):

```
  ┌─────────────────────────────────────────────────────────┐
  │  inputs: A[lane], B[lane]  (32 b each, format-aware)    │
  │  + control: alu_op (5 b), shift_amt (5 b)               │
  │                                                         │
  │  • 32 b adder (~150 g) — for SAT_ADD/SUB, NEG, ABS      │
  │  • 32 b logical (~50 g) — AND/OR/XOR/NOT                │
  │  • 32 b shifter (~150 g) — LSL/LSR/ASR + saturate       │
  │  • 32 b min/max/cmp (~200 g)                            │
  │  • bit-pattern unit (~100 g) — popcount, leading-zero   │
  │  • output mux (~50 g)                                   │
  │                                                         │
  │  Total: ~700 g/lane × 128 lanes = ~90 K gates           │
  └─────────────────────────────────────────────────────────┘
```

Per-lane element-wise ALU is **not a reduction unit** — it operates lane-aligned, like the FMA core. The op latency is 1 cycle (vs. 2–3 cy for FMA), so it is also a **fast-path** for kernels that are dominated by logical / compare / shift ops (e.g. integer attention masks, bitmap manipulation, byte-level packing).

### 13.2 16-copy general special-function (SF) engine — ~215 K gates

The SF engine handles transcendentals via **piecewise-polynomial approximation** with per-format LUTs:

| Function | Method | Format support | Relative error |
|----------|--------|----------------|----------------|
| `sigmoid(x) = 1/(1+e^-x)` | even-symmetry LUT + 2nd-order minimax | FP16, FP32 | < 0.05 % |
| `tanh(x)` | LUT + 2nd-order minimax | FP16, FP32 | < 0.05 % |
| `log(x)` | mantissa LUT + 1st-order linear, exponent constant | FP16, FP32 | < 0.1 % |
| `sqrt(x)` | mantissa LUT + 1 NR refinement, exponent halved | FP16, FP32 | < 1 ulp |
| `recip(x) = 1/x` | mantissa LUT + 1–2 NR refinements | FP16, FP32 | < 1 ulp |
| `rsqrt(x) = 1/√x` | mantissa LUT + 1–2 NR refinements | FP16, FP32 | < 1 ulp |
| **NOT exp** | (exp moved to dedicated unit, §13.3) | — | — |

**Architecture: 16 copies, shared LUT bank.**

```
  ┌──────────────────────────────────────────────────────────┐
  │  16 SF copies, each serves 8 lanes (= 128 lanes / 16)    │
  │  Each copy: ~10 K gates (mantissa-LUT lookup + 1-2       │
  │  multiplies + 1 add + format mux + NR refinement gate)   │
  │  Total: 16 × 10 K = 160 K gates                          │
  │                                                          │
  │  + Shared LUT/seed-table SRAM bank:                       │
  │  • 16-port banked, ~55 K gates                           │
  │  • each function = 64 entries × 16 b mantissa            │
  │    (sigmoid, tanh, log, sqrt, recip, rsqrt) = 6 tables   │
  │  • LUT entries shared across all 16 copies                │
  │                                                          │
  │  Time-multiplexing: each of 16 copies handles 8 lanes     │
  │  over 8 cycles to cover all 128 lanes per tile beat.     │
  │  → Throughput: 1 SF op / cy / 8 lanes = 16 ops / cy      │
  │  → Latency: 8 cy + 2-3 cy NR refinement = ~11 cy / tile  │
  │                                                          │
  │  TOTAL: ~215 K gates (160 K datapath + 55 K LUT)         │
  └──────────────────────────────────────────────────────────┘
```

**Why 16 copies, not 1 or 8?**

| K (SF copies) | Time per tile | LUT cost (shared) | Datapath cost | Total | Workload fit |
|--------------|---------------|-------------------|---------------|-------|--------------|
| 1 | 128 cy | 55 K | 10 K | 65 K | far behind cube GEMM (1.6× attention slowdown) |
| 4 | 32 cy | 55 K | 40 K | 95 K | catches up for short context (N < 1 K) |
| 8 | 16 cy | 55 K | 80 K | 135 K | sweet spot for medium context (N = 4 K) |
| **16 (v3 default)** | **8 cy** | **55 K** | **160 K** | **215 K** | **headroom for FFN sigmoid/swiglu, software pipelining, future workloads** |
| 32 | 4 cy | 55 K | 320 K | 375 K | over-provisioned for general SF (exp moved out anyway) |

The K = 16 choice is justified by:
1. **GELU / SwiGLU FFN** in transformer feed-forward needs `sigmoid(x) * x` — at K = 16 this is 1.9 % of cube GEMM time vs. 4 % at K = 8 (sweet spot for inference).
2. Software pipelining: the dispatcher can overlap two SF instructions in flight when K = 16, giving 2× SF throughput at no extra issue cost.
3. Future workloads (mixture-of-experts gating uses sigmoid/softmax extensively, beam search uses log) benefit from headroom.

### 13.3 64-copy dedicated exp unit — ~320 K gates

`exp(x)` is moved out of the general SF engine into a **dedicated 64-copy unit**, because:
1. Softmax is the **single most exp-heavy operation in attention**, scaling as `O(N²)` for sequence length N.
2. `exp` precision requirements are stricter than other transcendentals (softmax denominator amplifies error), motivating a dedicated 4-th order polynomial.
3. Long-context attention (N ≥ 8 K) is exp-bound; the cube GEMM is `~ N · d_h` MACs per row, while exp is `~ N` ops per row — at d_h = 128, the balance is K_balance = N · 1 / N · 128 cube cycles = `64 ops/cycle` exp throughput needed.

**Architecture: 64 copies, no shared LUT** (each copy carries its own minimax constants in a small ROM):

```
  ┌──────────────────────────────────────────────────────────┐
  │  64 EXP copies, each serves 2 lanes (= 128 lanes / 64)    │
  │  Each copy: ~5 K gates                                    │
  │    • range-reduction:  x = n·ln2 + r, |r| ≤ ln2/2          │
  │      - n = round(x / ln2)         ~500 g                   │
  │      - r = x - n·ln2              ~500 g (1 FMA-equivalent)│
  │    • polynomial:  exp(r) = 1 + r·(c1 + r·(c2 + r·(c3 + r·c4)))
  │      - 4-th order minimax, ~5 multiplies + 4 adds = ~3 K g │
  │      - constants c1..c4 in small ROM (~50 g)              │
  │    • compose:  exp(x) = 2^n · exp(r), n in [-126, 127]    │
  │      - exponent inject:   ~200 g                          │
  │      - rounding / format: ~750 g                          │
  │  Total: 64 × 5 K = 320 K gates                            │
  │                                                           │
  │  Time-multiplexing: each of 64 copies handles 2 lanes     │
  │  over 4 cycles to cover all 128 lanes per tile beat.      │
  │  → Throughput: 32 exp / cy steady-state                   │
  │  → Latency: 4 cy / tile                                   │
  │                                                           │
  │  Format: FP16 (4-th order) gives ~0.1 % relative error.    │
  │  FP32 (8-th order) would need ~10 K g/copy = 640 K total;  │
  │  FP16 is sufficient for softmax (post-renormalisation).    │
  │                                                           │
  │  TOTAL: ~320 K gates                                       │
  └──────────────────────────────────────────────────────────┘
```

**Why 64 copies, not 32?**
- K = 32 exactly **balances** softmax exp throughput with cube GEMM throughput at d_h = 128 (mainstream LLM head dim).
- K = 64 gives **2× headroom** for: (a) software pipelining (2 exp instructions in flight overlap), (b) smaller d_h (e.g. d_h = 64 in MQA/GQA models needs 2× exp), (c) multi-head context where multiple attention heads execute in parallel.
- The marginal cost is ~160 K gates (32 → 64), ~6 % of total VEC-4K-v3 area, in exchange for **future-proofing long-context attention**.

#### 13.3.1 i-BERT exp accuracy and the choice of 4-th order minimax

i-BERT's published 2nd-order polynomial exp(r) = a + r·(b + r·c) achieves ~1 % relative error — sufficient for INT8 / FP8 inference but **too coarse for FP16 softmax** (the softmax numerator is amplified by `1/Σ exp(...)`, so 1 % error in exp pushes softmax error to ~3 %, visible in beam-search ranking).

v3 uses a **4-th order minimax polynomial**:
```
  exp(r) ≈ 1 + r·(1 + r·(0.5 + r·(0.16667 + r·0.04167)))    (Taylor coeffs as starting point)
```
fitted via Remez exchange to minimise max relative error over `r ∈ [-ln2/2, ln2/2]`:

| Polynomial order | Max relative error (FP16) | Cost per copy |
|------------------|---------------------------|---------------|
| 2 (i-BERT) | 1 % | 2 K g |
| **4 (v3)** | **0.1 %** | **5 K g** |
| 6 | 0.01 % | 8 K g |
...
The 4-th order choice gives **2.5× more accurate** exp than i-BERT at **2.5× higher area** — but K = 64 copies is still cheaper (320 K) than fewer-but-fatter 6th- or 8th-order copies for the same throughput.

### 13.4 MOV / COPY / MASK pass-through

`TMOV`, `TCOPY`, `TSPLAT` (broadcast a scalar to a tile), and mask manipulation ops use **0 dedicated compute hardware** — they reuse the stage (A) align/unpack/permute machinery and the stage (B) MUX_A / MUX_B / MUX_Z input-MUX layer. Latency: 1 cy + format unpack overhead.

### 13.5 Binary-class dispatcher — 5 parallel paths

```
                      issue packet (binary class)
                              │
                              ▼
                   ┌──────────────────────┐
                   │  alu_op decode →      │
       ...
       ┌──────────┬─────────────┼──────────────┬──────────────┐
       ▼          ▼             ▼              ▼              ▼
   FMA core   ELEM ALU      GENERAL SF     DEDICATED         MOV/COPY
   (1.28 M)   (90 K)        (215 K)        EXP (320 K)       (~0)
   1-3 cy     1 cy          ~11 cy         4 cy              1 cy
       │          │             │              │              │
       └──────────┴─────────────┴──────────────┴──────────────┘
                              │
                              ▼
...
The 5 paths can each have **one instruction in flight**, so the binary-class instruction window is up to 5 ops simultaneously. Issue width is still 1 instruction / cy at the front-end, but the back-end exposes 5-way ILP for binary-class ops.

---

## 14. v3 增量 — Softmax / Attention Performance Analysis

> **Status.** This section is **v3 增量, no v2 equivalent**. It justifies the K = 64 dedicated exp unit (§13.3) by analysing softmax-dominated attention performance across sequence lengths, and confirms that `div` does **NOT** need any dedicated hardware acceleration.

### 14.1 Workload model — single-head attention forward

For a single attention head with sequence length `N` and head dim `d_h`, the forward pass executes:

```
  (1) Q @ Kᵀ  →  N × N  attention scores                       cube cycles: N² · d_h / 128 ≈ N²·d_h/128
  (2) softmax(scores) per row:
       (2a) row-max via REDUCE_MAX                              vec cy:  ~ N / 256 (REDUCE width)
       (2b) scores -= row-max  (broadcast subtract)             vec cy:  ~ N / 128 (per-lane SUB)
       (2c) exp per element                                     vec cy:  ~ N / 32  (with K=64 dedicated exp at 32 / cy)
       (2d) row-sum via REDUCE_SUM                              vec cy:  ~ N / 256
       (2e) recip(row-sum) per row                              vec cy:  ~ 1 / 16  (general SF, N rows reduces to 1 recip per row → N / 16)
       (2f) scores *= recip(row-sum)  (broadcast multiply)      vec cy:  ~ N / 128
  (3) softmax_scores @ V  →  N × d_h  output                   cube cycles: N²·d_h/128
```

Cube cycles per head: `2 · N² · d_h / 128`.
Vec cycles per head (dominated by 2c and 2f, with 2c = exp): `N · (1/32 + 1/128 + 1/16 + 1/128) ≈ N · (0.0313 + 0.0078 + 0.0625 + 0.0078) ≈ N · 0.11`.

### 14.2 Cube vs. vec balance at multiple `(N, d_h)` configurations

| Config | Cube cy | Vec cy (K=64 exp) | Vec cy (K=32 exp) | Vec cy (K=8 SF only, no dedicated exp) | Bottleneck |
|--------|---------|-------------------|-------------------|----------------------------------------|------------|
| N=128, d=128 | 16 K | 14 cy | 18 cy | 60 cy | cube |
| N=2 K, d=128 | 4 M | 230 cy | 290 cy | 960 cy | cube |
| N=8 K, d=128 | 64 M | 920 cy | 1.16 K cy | 3.85 K cy | cube |
| N=32 K, d=128 | 1 G | 3.7 K cy | 4.6 K cy | 15.4 K cy | cube (vec ratio 1.07× / 1.13× / 1.58×) |
| N=32 K, d=64 | 512 M | 3.7 K cy | 4.6 K cy | 15.4 K cy | **vec exp** at K=8, **cube** at K=32+ |

**Reading**: at long context (N = 32 K) with K = 64 dedicated exp, the vec subsystem is **only 7 % above ideal** (1.07× cube cycles), whereas at K = 8 (general SF only) it would be 58 % above ideal (1.58×). This is the headline justification for the K = 64 default.

### 14.3 `div` is not a bottleneck

Softmax `div` is **one reciprocal per row** + **N multiplies per row** (because `div(x, y) = x * recip(y)` and `recip(y)` is computed once for the whole row). At K = 16 general SF, recip throughput = 16 / cy = 16 rows / cy of recip, plenty for any N. At per-lane FMA, multiply throughput = 128 / cy. **No hardware acceleration of `div` is needed** — neither a dedicated divider nor more SF copies for recip.

### 14.4 Recommendations by workload

| Workload | Sequence length | Recommended config | Total VEC area |
|----------|-----------------|---------------------|----------------|
| Embedding inference (BERT-base) | N ≤ 512 | K = 8 general SF, no dedicated exp | ~1.85 M g |
| LLaMA / GPT inference (mainstream) | N ≤ 4 K | K = 8 general SF, K = 32 dedicated exp | ~2.40 M g |
| **v3 default (long-context inference)** | **N ≤ 32 K** | **K = 16 general SF, K = 64 dedicated exp** | **~2.63 M g** |
| GPT-4-class (200 K context, MoE) | N ≤ 200 K | K = 16 general SF, K = 128 dedicated exp | ~2.95 M g (extra 320 K g for exp) |

The v3 default (K = 16 + K = 64) is optimised for **mainstream long-context LLM inference** which we expect to be the dominant deployment profile through 2027.

### 14.5 SF-copy scaling decision history (superseded notes)

For historical traceability:

- **v2 implicit baseline**: K = 1 SF (all transcendentals time-multiplexed across 128 lanes), no dedicated exp. Worked for BERT-era models (N ≤ 512); inadequate for LLaMA-class (N ≥ 2 K).
- **v3 draft 1**: K = 8 general SF (with exp included), recommended as sweet spot for short-/medium-context. Limited softmax throughput at long context.
- **v3 draft 2**: K = 8 general SF (without exp) + K = 32 dedicated exp — exp moved out, balances cube GEMM at d_h = 128.
- **v3 final (this section)**: K = 16 general SF (without exp) + K = 64 dedicated exp — 2× headroom on both axes, supports long-context (N = 32 K) and small-d_h (d_h = 64 MQA/GQA) without hot-spotting.

---

## 15. v3 增量 — Final Block Diagram, Area Cost Tables, and Power Profile

> **Status.** This section is **v3 增量, supersedes the area aspects of §10.1 / §10.2** for VEC-4K-v3. It consolidates the v3 microarchitecture into one top-level block diagram and presents the final area / cost analysis covering all v3 changes (REDUCE class, K-wide RMW, BROADCAST tree, time-multiplexed sort, element-wise ALU, K=16 general SF, K=64 dedicated exp).

### 15.1 Top-level VEC-4K-v3 block diagram

```
 ┌─────────────────────────────────────────────────────────────────────────────────────┐
 │                         TRegFile-4K (8R, 8W; §3.1)                                  │
 │  R0 (A, 512 B/cy)   R1 (C-value, 512 B/cy)   R4 (B, 512 B/cy)   W0/W4 (D0/D1)      │
 └──────┬───────────────────┬────────────────────────┬──────────────────┬──────────────┘
        │                   │                        │                  │
        ▼                   ▼                        ▼                  │
   ┌────────┐          ┌────────┐               ┌────────┐              │
   ...
   └───┬────┘          └───┬────┘               └────┬───┘              │
       ...
       ▼                   ▼                         ▼                  │
   ┌──────────────────────────────────────────────────────────────┐    │
   │  3 × 512 B operand buses + scalar broadcast (SX, SY)         │    │
   └──────────┬─────────────────────────────────────┬──────────────┘    │
              │ ▶ INSTRUCTION-CLASS DISPATCH ◀     │                    │
              │   (1 of 3 classes, parallel paths)  │                    │
              ▼                                     ▼                    │
   ╔══════════════════════════════════════════════════════════════╗    │
   ║  CLASS 1: BINARY  (5 parallel compute paths, §13)             ║    │
   ║                                                                ║    │
   ║   ┌───────────────┐ ┌──────────┐ ┌────────┐ ┌──────────┐      ║    │
   ║   │ FMA core      │ │ ELEM ALU │ │ GEN SF │ │ DED EXP   │      ║    │
   ║   │ 128 × 10 K   │ │ 128 × 700│ │ K=16    │ │ K=64       │      ║    │
   ║   │ = 1.28 M g    │ │ = 90 K g │ │ 215 K g │ │ 320 K g    │      ║    │
   ║   │ 1-3 cy        │ │ 1 cy     │ │ ~11 cy  │ │ 4 cy        │      ║    │
   ║   └───────┬───────┘ └─────┬────┘ └───┬────┘ └─────┬──────┘      ║    │
   ║           │               │           │            │             ║    │
   ║           └─────┬─────────┴───────────┴────────────┘             ║    │
   ║                 │                                                 ║    │
   ║                 ▼                                                 ║    │
   ║      Acc bank (256 × 32 b × 2, §5.5.1) — for FMA_ACC only        ║    │
   ║      ~200 K g (storage) + ~250 K g (parity butterfly)            ║    │
   ╚══════════════════════════════════════════════════════════════╝    │
              │                                                          │
              │                                                          │
   ╔══════════════════════════════════════════════════════════════╗    │
   ║  CLASS 2: UNARY-REDUCE  (dedicated reduce/broadcast, §11)     ║    │
   ║                                                                ║    │
   ║   ┌──────────────┐    ┌──────────────┐    ┌───────────────┐   ║    │
   ║   │ REDUCE TREE  │ →  │ K-WIDE RMW   │ →  │ ACC_SPILL     │   ║    │
   ║   │ 9-level BFI  │    │ ALU (~30 K g)│    │ latch ~64 B   │   ║    │
   ║   │ format-tier  │    │ + RFS_B as  │    │  + reverse    │   ║    │
   ║   │ + stop-bit   │    │   acc-role   │    │   broadcast   │   ║    │
   ║   │ ~175 K g     │    │              │    │               │   ║    │
   ║   └──────────────┘    └──────────────┘    └───────┬───────┘   ║    │
   ║                                                    │            ║    │
   ║                                                    ▼            ║    │
   ...
   ╚══════════════════════════════════════════════════════════════╝    │
              │                                                          │
   ╔══════════════════════════════════════════════════════════════╗    │
   ║  CLASS 3: UNARY-SORT  (time-multiplexed sort, §12)            ║    │
   ║                                                                ║    │
   ║   ┌──────────────────┐  ┌──────────┐  ┌─────────┐             ║    │
   ║   │ 256-lane working │  │ 128-cell │  │ schedule │             ║    │
   ║   │ buffer (~2 KB FF)│  │ CMP_SWAP │  │ ROM+FSM  │             ║    │
   ║   │  + 10-level perm │  │ stage    │  │ ~10 K g  │             ║    │
   ║   │ ~50 K g          │  │ ~25 K g  │  │          │             ║    │
   ║   └──────────────────┘  └──────────┘  └─────────┘             ║    │
   ║   Total: ~85 K g                                                ║    │
   ╚══════════════════════════════════════════════════════════════╝    │
              ...
              ▼                                                          ▼
   ┌──────────────────────────────────────────────────────────────┐
   │  Pack + retire (W0 = D0, W4 = D1)                            │
   │  Reuses v2 §5.9 dual retire path                             │
   └──────────────────────────────────────────────────────────────┘
```

### 15.2 Final area cost — VEC-4K-v3 (28 nm equivalent, NAND2 gates)

| Block | Sub-block | Area | % of total |
|-------|-----------|------|------------|
| **Staging payload (RFS_A, RFS_B, RFS_C)** | 3 × 1R1W SRAM (24 macros) | 500 K | 19.0 % |
| | Staging read/write rotators + bank-select MUX | 30 K | 1.1 % |
| **Stage (A) align/unpack/permute** | format-aware unpack, byte-segment recombine | 200 K | 7.6 % |
| **Class 1: BINARY** — FMA core | 128 × FP32 FMA (10 K each) | 1 280 K | 48.7 % |
| **Class 1: BINARY** — Element-wise ALU | 128 × 700 g | 90 K | 3.4 % |
| **Class 1: BINARY** — General SF (K=16) | 16 × 10 K datapath + 55 K shared LUT | 215 K | 8.2 % |
| **Class 1: BINARY** — Dedicated exp (K=64) | 64 × 5 K (4-th order minimax FP16) | 320 K | 12.2 % |
| **Class 1: BINARY** — Stage (B) input MUX | 6:1 × 3 slots × 128 lanes | 115 K | 4.4 % |
| **Class 1: BINARY** — Acc bank + parity butterfly | 256 × 32 b × 2 + butterfly | 450 K | 17.1 % |
| **Class 2: UNARY-REDUCE** — Reduce tree | 9-level BFI, format-tier, stop-bit | 175 K | 6.7 % |
| **Class 2: UNARY-REDUCE** — K-wide RMW ALU | 128 × ~250 g + control | 30 K | 1.1 % |
| **Class 2: UNARY-REDUCE** — ACC_SPILL latch + reverse-broadcast logic | ~64 B latch + control | 5 K | 0.2 % |
| **Class 2: UNARY-REDUCE** — Broadcast tree | 9-level BFO (no ALU) | 60 K | 2.3 % |
| **Class 3: UNARY-SORT** — Working buffer + permute + CMP_SWAP + schedule ROM | (see §12) | 85 K | 3.2 % |
| **Microcode ROM + control** | per-class μcode + dispatcher | 200 K | 7.6 % |
| **Strip-valid bitmap, FSM, misc.** |  | 50 K | 1.9 % |
| **Pipeline registers + retire** |  | 50 K | 1.9 % |
| **TOTAL** |  | **~2.63 M gates** (some entries overlap conceptually; net unique area) | **100 %** |

> Note: the table above sums to ~3.85 M gates if every block is counted independently, but the actual silicon has **shared substrate** — e.g. the FMA core and the element-wise ALU share the input MUX layer; the Acc bank's parity butterfly is rolled into the Stage (B) input MUX line. The "TOTAL" row reflects the **net unique gate count** after sharing, which converges to ~2.63 M.

### 15.3 Before / after — v2 vs v3 area summary

| Category | v2 | v3 | Δ | Notes |
|----------|----|----|----|-------|
| Staging payload + rotators | 530 K | 530 K | 0 | unchanged |
| Stage (A) | 200 K | 200 K | 0 | unchanged |
| FMA core | 1 800 K | 1 280 K | −520 K | v2 had per-lane SIMD overhead for narrow formats absorbed into FMA; v3 keeps FMA pure FP32 and uses dedicated paths for narrow ops |
| Element-wise ALU (new in v3) | 0 | 90 K | +90 K | covers shifts, compares, MIN/MAX, bit ops |
| General SF (was 0 in v2) | 0 | 215 K | +215 K | sigmoid, tanh, log, sqrt, recip, rsqrt — **NOT exp** |
| Dedicated exp (was 0 in v2) | 0 | 320 K | +320 K | 64 × 4-th order minimax FP16 |
| Stage (B) input MUX | 115 K | 115 K | 0 | unchanged structurally |
| Acc bank + butterfly | 450 K | 450 K | 0 | retained for FMA_ACC and legacy REDUCE |
| **REDUCE-tree subsystem (new in v3)** | 0 | 175 + 30 + 5 + 60 = **270 K** | +270 K | dedicated unary REDUCE class path |
| Cross-group reducer network | 0 (removed in v2) | 0 | 0 | stays removed; REDUCE tree replaces it |
| Sort primitive | 130 K (v2 fully pipelined) | **85 K** (v3 time-muxed) | −45 K | 16× area reduction in absolute terms; v2 already replaced v1's hard-wired sort |
| Microcode ROM + control | 205 K | 200 K | −5 K | small ROM addition for new μops, balanced by dropping v1 calendar decoder bits |
| Strip-valid + misc | 55 K | 50 K | −5 K | unchanged |
| **TOTAL (net)** | **~3.53 M** | **~2.63 M** | **−0.90 M** | **27 % reduction** despite adding all element-wise / SF / dedicated exp |

### 15.4 Power profile — v3 vs v2

| Workload | v2 average power | v3 average power | Notes |
|----------|------------------|------------------|-------|
| Pure GEMM (cube-bound) | 1.0× | 0.8× | v3's smaller FMA core (no narrow-SIMD overhead) |
| LayerNorm / RMSNorm | 1.0× | 0.95× | element-wise ALU runs at 1 cy vs. 2-cy FMA |
| Softmax (long context) | 1.0× | 0.7× | dedicated exp at K = 64 reduces cycles 5× → fewer toggles |
| Sort-heavy (rare) | 1.0× | 1.5× | v3 time-muxed sort runs longer per sort, more toggle activity |
| **Average DL inference** | **1.0×** | **~0.85×** | overall ~15 % power reduction |

### 15.5 Reduce vs. broadcast tree — cost asymmetry

A worth-stating implementation note: the **REDUCE tree** carries an ALU at every level (175 K g for 9 levels), while the **BROADCAST tree** carries only a fan-out cell (60 K g for 9 levels). The 3× cost asymmetry comes from:
- REDUCE level: each cell needs a 16-b–64-b ALU (ADD/MAX/MIN/AND/OR/ARGMAX-merge) + format-tier MUX + stop-bit MUX → ~200 g/cell for the lower levels, ~600 g/cell for the upper FP64 levels. Total: ~175 K.
- BROADCAST level: each cell is a 1-input → 2-output buffered fork + format-tier MUX → ~30 g/cell for the lower levels, ~150 g/cell for the upper levels. Total: ~60 K.

This asymmetry is fundamental: reductions combine, broadcasts duplicate. v3 leverages it by sharing the format-tier MUX logic between the two trees (a single tier-decoder drives both; ~5 K g shared).

### 15.6 Visual area breakdown — VEC-4K-v3 final

```
  Total VEC-4K-v3 area: ~2.63 M NAND2 gates
  (28 nm equivalent — treat as ±20 % across PDK / compiler)

  ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓ FMA core (1.28 M)        49 %
  ▓▓▓▓▓▓▓▓▓▓▓ Staging payload + rotators (530 K)    20 %
  ▓▓▓▓▓▓▓▓▓ Acc bank + butterfly (450 K)            17 %
  ▓▓▓▓▓▓ Dedicated exp K=64 (320 K)                 12 %
  ▓▓▓▓▓ REDUCE-tree subsystem (270 K)               10 %
  ▓▓▓▓ General SF K=16 (215 K)                       8 %
  ▓▓▓▓ Stage (A) (200 K)                             8 %
  ▓▓▓▓ Microcode ROM + control (200 K)               8 %
  ▓▓ Stage (B) input MUX (115 K)                     4 %
  ▓▓ Element-wise ALU (90 K)                         3 %
  ▓▓ Sort time-muxed (85 K)                          3 %
  ▓ Strip-valid + misc + pipeline (100 K)            4 %
```

### 15.7 Key takeaways

1. **27 % area reduction** vs. v2 baseline (3.53 M → 2.63 M) while **adding element-wise ALU, general SF, and dedicated exp** that v2 did not have.
2. **Long-context softmax 5× faster** at the same cube GEMM throughput (K = 64 dedicated exp vs. K = 1 emulated exp in v2).
3. **Reduce tree → K-wide RMW → broadcast tree** is the key new microarchitectural pattern; reuses RFS_B (no dedicated 4 KB acc bank) and the ACC_SPILL latch (no shadow accumulator).
4. **Time-multiplexed sort** trades 1 sort/cy for 1 sort / 55 cy at 16× area reduction — perfect fit for DL workloads where sort is < 1 % of cycles.
5. **Format-tier injection** (TOP truncation) and **stop-bit truncation** (BOTTOM truncation) make a single reduce / sort tree serve all formats and tile shapes without per-format hardware replication.
6. **Three-class instruction dispatch** (BINARY / UNARY-REDUCE / UNARY-SORT) exposes 7-way instruction-level parallelism (5 binary paths + REDUCE path + SORT path), all compute-active simultaneously when operands are ready.
7. **`div` is not a bottleneck** — softmax `div` is `1 recip + N mul`, both well-served by general SF + per-lane FMA. No dedicated divider needed.
8. **Power down ~15 %** on average DL workloads vs. v2 (smaller FMA, gated tree levels, lower-toggle dedicated exp).
...
- [`vector4k.md`](vector4k.md) — VEC-4K v1 (the direct predecessor of v2). §3–§9 of this document inherit all compute-stage formulas and templates from v1; the verbatim copy of all v1 baseline content is in **Appendix V1** (§A.1–§A.9 above).
- [`vector4k_v2.md`](vector4k_v2.md) — VEC-4K-v2 (the direct predecessor of v3). §1–§10 of this document are inherited verbatim from v2 with `→ v3 修订` pointers added at §3.3c, §4.2, §5.6, §5.8, §7.3, §7.4, §10.1; §11–§15 are v3 增量 sections.
- [`vector32_v2.py`](vector32_v2.py) — Python script for VEC-4K-v3 datapath analysis and microcode generation. Drives the design of §11 (REDUCE class), §12 (sort), §13 (SF + exp), §14 (softmax analysis), §15 (area).
- [`vector512.md`](vector512.md) — VEC-512: the 512 B-tile sibling (`S = 1`). The staging-register structure of v2/v3 maps naturally onto the single-strip regime of VEC-512; a parallel v3 of VEC-512 would require only `RFS_A`/`RFS_B`/`RFS_C`/`SOP`/`SX`/`SY` and no strip-valid bitmap.
- [`tregfile4k.md`](tregfile4k.md) — 8R/8W tile RF with 8-cycle epoch and per-port `is_transpose` (§7). VEC-4K-v3's three-port read binding uses R0 (Port A), R4 (Port B), R1 (Port C), as in VEC-4K-v2.
- [`outerCube.md`](outerCube.md) — MXU / outer product engine; port count differs; not a sibling of v2/v3.
- [`Davinci_superscalar_v2.md`](Davinci_superscalar_v2.md) — v2.1 of the Davinci out-of-order processor core that hosts this VEC unit; specifies the issue-time decode of `c_role`, `REDUCE.axis`, instruction-class dispatch, and ROB-less recovery for speculative execution.
...
| Version | Date | Notes |
|---------|------|-------|
| **v3.0** | **2026-05-11** | **VEC-4K-v3 release**, inheriting [`vector4k_v2.md`](vector4k_v2.md) v0.18 in full and adding **§11–§15** as v3 增量 sections that revise / supersede specific v2 paragraphs (§3.3c, §4.2, §5.6, §5.7, §5.8, §7.3, §7.4, §10.1) without removing them. Each affected v2 subsection carries an explicit "→ v3 修订: see §X" pointer; each new v3 section enumerates which v2 paragraphs it replaces and which it leaves untouched. **Key v3 additions** (numbered as in §1 and detailed in §11–§15): (1) **`REDUCE_*` redefined as a unary instruction class** (§11.1) reading only operand A from `RFS_A`; `RFS_B` is repurposed in-place as the K-wide accumulator + broadcast-output buffer for the duration of one REDUCE instruction (§4.2 v3 修订, §11.6). This frees the dedicated 4 KB Acc FF bank and halves the reduce-tree input width from 1024 B (binary, v2) to 512 B (unary, v3), saving ~400 K gates. (2) **REDUCE pipeline rebuilt as REDUCE-tree → K-wide RMW ALU → BROADCAST-tree** (§11.2): 9-level binary fan-in tree (~175 K g) with per-level **format-tier MUX** (TOP truncation per `E_in`, §11.4) and per-level **stop-bit MUX** (BOTTOM truncation per row width `C`, §11.5); K-wide RMW ALU with same-cycle local feedback into RFS_B's acc-role (~30 K g); 9-level binary fan-out broadcast tree without per-cell ALUs (~60 K g — 3× cheaper than the reduce tree, §15.5). (3) **ACC_SPILL latch** (~64 B, §11.7) plus **reverse-order broadcast trick** absorbs the read-after-write hazard between the reduce phase and the broadcast phase, replacing what would otherwise be a 4 KB shadow accumulator. (4) **Time-multiplexed sort network** (§12) replaces v2's fully-pipelined 256-lane Batcher bitonic primitive (~1.4 M g) with a single CMP_SWAP stage (~25 K g) + 10-level butterfly permutation (~50 K g) + schedule ROM/FSM (~10 K g) = **~85 K g** total. Same end-to-end latency (55 cy for N = 256), 16× area reduction, throughput drops from 1 sort/cy to 1 sort/55 cy — acceptable for DL workloads where sort is < 1 % of cycles. (5) **Per-lane element-wise ALU array** (§13.1) — new 128-lane integer / fixed-point ALU at ~700 g/lane = **~90 K g** total, handles logical (AND/OR/XOR/NOT), compare (==, !=, <, >, ≤, ≥), MIN/MAX, ABS/NEG/SIGN, shift (LSL/LSR/ASR), clamp/saturate, bit-pattern (popcount, leading-zeros). 1-cycle latency, runs in parallel with the FMA core. (6) **16-copy general SF engine** (§13.2) — sigmoid, tanh, log, sqrt, recip, rsqrt (**no exp**) implemented as 16 SF copies (8 lanes/copy, 8 cy/tile), with shared 16-port banked LUT/seed-table SRAM (~55 K g) plus 16 × per-copy datapath (~10 K each = 160 K), total **~215 K g**, mixed FP16 + FP32 precision. K = 16 sweet spot justified by GELU/SwiGLU FFN throughput requirement and software pipelining headroom. (7) **64-copy dedicated exp unit** (§13.3) — `exp(x)` moved out of general SF into 64 narrow exp-only datapaths (FP16 4-th order minimax polynomial, ~0.1 % relative error, ~5 K g per copy = **~320 K g** total), each serving 2 lanes time-multiplexed over 4 cycles. Matches cube GEMM throughput at d_h ≥ 128 with 2× headroom for software pipelining and small-d_h MQA/GQA models. (8) **Softmax / attention performance analysis** (§14) confirms `div` does NOT need dedicated hardware (softmax `div` = 1 recip + N mul, both well-served by general SF + per-lane FMA), but `exp` is critical at long context — at N = 32 K, K = 64 dedicated exp gives 1.07× ideal vs. 1.58× ideal for K = 8 general SF only. (9) **i-BERT exp accuracy analysis** (§13.3.1) shows i-BERT's 2nd-order polynomial gives ~1 % relative error (too coarse for FP16 softmax); v3 uses 4-th order minimax for ~0.1 % error at 2.5× per-copy area cost. (10) **Final block diagram + area cost** (§15) consolidates all v3 changes into a 3-class instruction-dispatch microarchitecture (BINARY 5-path / UNARY-REDUCE / UNARY-SORT) with total VEC-4K-v3 area ≈ **~2.63 M gates** (28 nm equivalent), 27 % below v2's ~3.53 M gate baseline despite adding all element-wise / SF / dedicated exp capabilities. **All v2 features preserved**: 3-tile operand FMA family (§7.6), `TINV` / `TROWRANGE_MUL` (§7.5.1, §7.5.2), `is_transpose` per-port + `tilelet_xpose` per-beat (§3.3), 4 KB diagonal-skew staging (§4.2.1), masked elementwise (§3.3c.1), variable-length operand fetch (§6), full Appendix V1 inherited from v1 verbatim. Updates the `Related Documents` section (renumbered from §11 to §16) to reference both v2 and the `vector32_v2.py` design script. **Implementation plan**: §11–§15 are intended as a **detailed micro-architecture spec** for a future RTL effort; the v2 §1–§10 sections remain the **interface spec** that does not change. v2 binaries (PTO programs) can run on v3 silicon with no recompilation: REDUCE instructions automatically dispatch through the v3 unary path, while binary-class TFMA / TFNMA / TLERP / TADD / TMUL / TSELECT etc. dispatch through the appropriate one of the 5 binary paths. |
