...
| VTG `V*` micro-op | `V` | 256 B or 512 B VTG | One VTG per GVIQ entry | Strip-mined elementwise, inner loops |

The two execution paths share the same VEC-4K-v2 ALU and TRegFile-4K. They arbitrate for VEC staging and TRegFile ports via the existing issue pipeline. A VTG micro-op uses the **micro-instruction buffer** (pre-decoded) rather than the Vector RS opcode field.
...
The GVIQ holds VTG vector micro-instructions waiting for their source VTGs to become ready. It is **separate from** the existing Vector RS (24 entries, full-tile `T*` ops). The two paths share the VEC-4K-v2 ALU but have independent issue queues and issue logic.
...
The VEC-4K-v2 datapath supports two execution modes:
...
| **VTG `V*` micro-op** | One VTG micro-op | 256 B or 512 B | 1 or 2 SIMD group beats | VTG sub-range via Group Read/Write Adapters |

**VEC staging reuse for VTG micro-ops:**

| Staging Register | VTG Micro-Op Use |
|-----------------|-----------------|
| `SA` | Source VTG 0 (or old destination for merging predication) |
| `SB` | Source VTG 1 (or scalar broadcast expansion via SX/SY) |
| `SC` | Predicate VTG (or third source VTG for wide ops) |
| `SX / SY` | Scalar operand broadcast / loop counter broadcast |
| `SOP` | Micro-instruction opcode and control from micro-instruction buffer |
...
```text
input:  full_tile[4096 B], group_id, group_mode
G256:   vtg[256 B] = full_tile[group_id * 256 : (group_id+1) * 256]
G512:   vtg[512 B] = full_tile[group_id * 512 : (group_id+1) * 512]
output: vtg -> SA or SB staging register
```
...
G256:   full_tile = merge(vtg_result, existing_tile, group_id)  # preserves other 15 VTGs
G512:   full_tile = merge(vtg_result, existing_tile, group_id)  # preserves other 7 VTGs
TRegFile.write(dst_ptag, full_tile)
update VTG_metadata[dst_ptag][group_id] = {valid=1, defined=1, dirty=1}
```

**Micro-instruction buffer integration:** The `MicroOpEntry` from the buffer drives VEC staging muxing, ALU opcode, and element-type configuration. The buffer lookup happens at I1 simultaneously with TRegFile read.

**Paired `G256` issue (optional):** When two 256 B VTGs have matching `{opcode, elem_type, pred_mode}`, the VTG rotation scheduler may issue them together, filling the full 512 B SIMD group beat.
...
Each 4 KB tile register is partitionable into **Vector Thread Groups (VTGs)** for SIMD-group execution. The VTG is the scheduling, rename, and writeback unit for VTG vector micro-instructions.
...
**Rename:** The Tile RAT maps architectural tile `Tg` -> physical tile `PT`. VTG `group_id` is a sub-location index into the renamed `PT`. A VTG micro-instruction writing `Tg.gN` may either update `PT.gN` in place (if uniquely owned) or allocate a fresh physical tile and merge unchanged VTGs (copy-on-write policy, v1 conservative policy).
...
VTG vector micro-instructions have a two-level dependency model:
...
|--------|---------|--------|
| In-place VTG write | Unique tile ownership, no older readers | Write destination VTG sub-range; preserve other VTGs |
| Copy-on-write tile group | Shared tile or speculative update | Allocate fresh PT; merge unchanged VTGs; update Tile RAT |
...
VTG vector memory operations load or store 256 B or 512 B VTG payloads under predicate control.
...
3. Read predicate VTG (`T8.p0`) from TRegFile via Group Read Adapter
4. LSU fetches active lanes from memory (inactive lanes skipped)
5. Build 256 B or 512 B VTG payload
6. Write payload into `T8.g0` through Group Write Adapter
7. Update VTG metadata for `T8.g0` (`valid=1`, `defined=1`, `dirty=1`)
