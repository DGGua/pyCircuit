...
> **仓库位置**：Davinci 设计树（含 `davinci_top`）维护在独立仓库 [DavinciOO](https://github.com/hengliao1972/DavinciOO) 的**仓库根目录**（`davinci/`、`PTOISA/` 等），不再随 pyCircuit 发布。下列命令沿用 `python3 -m designs.outerCube.davinci.davinci_top` 时，请在 pyCircuit 根目录的 `designs/` 下提供被 `.gitignore` 忽略的 **`outerCube`** 名称（**符号链接**指向 DavinciOO 克隆的根目录即可），并把 `compiler/frontend` 加入 `PYTHONPATH`；详见 DavinciOO 内 `davinci/README.md`。
...
下面以 Davinci 乱序处理器核为例（源码见 [DavinciOO / `davinci`](https://github.com/hengliao1972/DavinciOO/tree/main/davinci)；需按上文用 `designs/outerCube` **符号链接**指向 DavinciOO 仓库根目录），展示层次化和扁平化两条完整构建流水线。
...
Davinci 核（[`DavinciOO` / `davinci/davinci_top.py`](https://github.com/hengliao1972/DavinciOO/blob/main/davinci/davinci_top.py)）展示了所有模式的大规模应用——27 个模块、1.5M+ 字符 MLIR：
