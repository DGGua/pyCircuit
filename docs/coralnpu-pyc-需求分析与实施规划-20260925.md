...
S4 已接上。整数向量是 `VLEN=128`、`SEW=32`、`LMUL=1`，四条 32 位通道。`vsetvli` 把 VLMAX=4 写入标量目的寄存器。`vadd.vv`、`vand.vv`、`vor.vv`、`vxor.vv` 在发射拍写回向量寄存器。`vle32.v` 和 `vse32.v` 共用 DTCM 端口，每拍一个字。`tb_rvv` 把 DTCM 里的 1、2、3、4 装入向量寄存器，做加法和逻辑运算后再存回去，`tohost` 为 20。形状对照 Coral `tests/cocotb/rvv/rvv_add.S` 里的 `vle` / `vadd.vv`，元素宽度改成 32 位。S1–S3 的五条测试仍然通过。矩阵 GEMM 仍未做。
