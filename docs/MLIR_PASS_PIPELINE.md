...
| 31 | `pyc-eliminate-dead-instances` | function | 再次删除优化后变得不可观察的实例。 | 始终 |
| 32 | `symbol-dce` | module | 删除无用符号。 | 始终 |
...
| 36 | `pyc-plan-change-driven-schedule` + `pyc-check-change-driven-schedule` | module | 为 fused comb、实例和状态源生成并校验 change-driven 调度。 | 始终 |
| 37 | `pyc-cpp-placement` | module | 为 C++ emit 标注 comb 方法切分与 struct/local 存储；trace codegen plan 会校验被采样字段。 | 仅 `--emit=cpp` |
| 38 | `pyc-collect-compile-stats` | function | 写入寄存器、存储和硬件位数等编译统计属性。 | 始终 |

通过 flat-types、no-dynamic、logic-depth 这三项 legality gate 后，`pycc` 才调用 C++ 或 Verilog emitter。
C++ 路径在 emit 前跑 `pyc-cpp-placement`。`--comb-partition=static` 时跳过 `pyc-fuse-comb`，改为 `pyc-partition-comb`。
`--module-pipeline=analyze|rewrite` 时，时钟域检查和 i1 pack 提前到 `pyc-module-pipeline` 之前，组合环检查留在其后。
